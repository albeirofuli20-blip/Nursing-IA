...
export default function Home() {
  ...
  async function load() { const user = await base44.auth.me(); const [patients, guides, allPlans, users] = await Promise.all([base44.entities.Patient.list("-updated_date"), base44.entities.Guide.list("-updated_date"), base44.entities.CarePlan.list("-updated_date"), user.role === "admin" ? base44.entities.User.list() : Promise.resolve([])]); const plans = user.role === "admin" ? allPlans : allPlans.filter(p => p.created_by_id === user.id); setData({ user, patients, guides, plans, users }); setLoading(false); }
  useEffect(() => { load(); }, []);
